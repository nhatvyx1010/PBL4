...
def get_Frame(ret, frame):
    ...
    if model_plates_path:
        print(model_plates_path)
        # Load a model
        model_plates = YOLO(model_plates_path)  # load a plates model
        model_charaters = YOLO(model_charaters_path) # load a charaters model
        threshold = 0.4
            ...
            for result in results.boxes.data.tolist():
                ...
                if score > threshold:
                    ...
                    if len(line_box) >= 7 and len(line_box) <= 9:
                        ...
                        return frame_out, new_img
    else:
        print("null model plate")
