...
def get_Frame(ret, frame, model_plates, model_charaters):
    H, W, _ = frame.shape
    kitu = ''
    ...
    for result in results.boxes.data.tolist():
        ...
        if len(line_box) >= 7 and len(line_box) <= 9:
            ...
            kitu = get_Characters(model_charaters, new_img)

            frame_out = resize(frame_out)
            new_img = resize(new_img)

            return frame_out, new_img, kitu
    
def get_Frame2(ret, frame, model_plates):
    H, W, _ = frame.shape
    threshold = 0.2
...
def line_detection(image):
    ...
    for i, (x, y, w, h) in enumerate(boundingBoxes):
        boundingBoxes[i] = (x - 3, y - 3, w + 6, h + 6)
...
def cut_and_paste(line_box, image):
    ...
    max_width = sum(box[2] for box in line_box) + 1 * (len(line_box) - 1)
    ...
    for box in line_box:
        ...
        x_offset += w + 1  # Khoảng cách giữa các box là 3
